...
deletion:
- lookup after deletion: found=true user={ID:42 Name:Ada}
- repository loads: 4

stats:
- entries=1 hits=2 misses=4
- loads_found=2 loads_not_found=2 load_errors=0
- deleted_entries=1 evictions=0 expirations=0
```
...
* **Deletion:** After `Delete(42)`, the next lookup loads the value from the repository again and stores the
